...
public class Reactor extends Bomb {
    ...
    public void removeItems(Inventory inv, ReactorData data) {
        ...
        removeOutput(inv, data, location);
        removeWaste(inv, data, location);
    }

    private void removeOutput(Inventory inv, ReactorData data, Location location) {
        if (inv.firstEmpty() == -1) {
            return;
        }
    ...
    private void removeWaste(Inventory inv, ReactorData data, Location location) {
        if (inv.firstEmpty() == -1) {
            return;
        }
