...
import com.mortisdevelopment.mortisnuclearcraft.nuclearcraft.centrifuge.Centrifuge;
import com.mortisdevelopment.mortisnuclearcraft.nuclearcraft.centrifuge.CentrifugeManager;
import com.mortisdevelopment.mortisnuclearcraft.nuclearcraft.centrifuge.CentrifugeRecipe;
...
public class CentrifugeConfig extends Config {
    ...
    public void loadConfig() {
        ...
        getConfigManager().getManager().setCentrifugeManager(new CentrifugeManager(getConfigManager().getManager().getDataManager(), centrifuge, menuItems));
    ...
    private void loadRecipes(ConfigurationSection recipes) {
        ...
        for (String key : recipes.getKeys(false)) {
            ...
            ConfigurationSection outputSection = section.getConfigurationSection("outputs");
            if (outputSection == null) {
                continue;
            }
            Randomizer<ItemStack> outputs = Randomizer.getItemRandomizer(getConfigManager(), outputSection);
            long duration = section.getLong("duration");
            int fuelPower = section.getInt("fuel-power");
            CentrifugeRecipe centrifugeRecipe = new CentrifugeRecipe(key, input1, input2, outputs, duration, fuelPower);
